...
window.MontanaChatIntent = (function () {
  ...
  function isAgreement(norm, raw) {
    ...
    return hasAny(norm, [
      /^ياريت/, /^يا ريت/, /^اتمنى/, /^خلاص/, /^يلا/, /^ماشي/, /^تمام/, /^اه/, /^ايوه/, /^نعم/,
      /^موافق/, /^اكيد/, /^حاضر/, /^حلو/, /^اوك/, /^اوكي/, /^ok\b/, /^yes/, /^طيب/, /^صح/,
      /^تمام اوي/, /^يلا بينا/, /^مظبوط/, /^خد$/, /^يلا كده/
    ]);
  }

  function scoreAffirm(norm) {
    if (isAgreement(norm, norm)) return 6;
    if (norm.length < 35 && /(?:^|\s)(ايوه|اه|تمام|نعم|ماشي|اكيد|ياريت|خلاص|يلا|حاضر|طيب|صح|مظبوط|اوكي)(?:\s|$)/.test(norm)) return 5;
    return 0;
  }
