-- Fix: pg-safeupdate blocked create_guest_order
-- Migration 106 used `UPDATE _order_lines SET ...` without WHERE,
-- which Supabase rejects with: "UPDATE requires a WHERE clause".
-- That made every checkout confirm fail with a generic error.
...
begin
  ...
  for v_bslug, v_bundle_price, v_required in
    select * from (values
      ('post-laser-glow', 549::numeric, array['post-laser-cream','whitening-cream']::text[]),
      ('brightening-routine', 499::numeric, array['whitening-cleanser','whitening-cream']::text[]),
      ('face-and-body', 499::numeric, array['acne-facial-cleanser','hand-body-lotion']::text[])
  ...
  loop
    ...
    -- Proportional allocation (same as js/bundles.js allocateBundleUnitPrices)
    v_alloc := array[]::numeric[];
    v_head := 0;
    v_i := 0;
    foreach v_slug in array v_required loop
      v_i := v_i + 1;
      select ol.list_price into v_product_price
      from _order_lines ol
      where ol.bundle_slug = v_bslug and ol.product_slug = v_slug
      limit 1;
      if v_i < array_length(v_required, 1) then
        v_unit := round(v_bundle_price * v_product_price / v_list_sum);
        v_alloc := v_alloc || v_unit;
        v_head := v_head + v_unit;
      else
        v_alloc := v_alloc || (v_bundle_price - v_head);
      end if;
    end loop;
    ...
    foreach v_slug in array v_required loop
      v_i := v_i + 1;
      update _order_lines
        set unit_price = v_alloc[v_i]
      where bundle_slug = v_bslug and product_slug = v_slug;
    end loop;
  end loop;
