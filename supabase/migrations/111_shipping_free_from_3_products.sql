-- =============================================================================
-- Montana: إصلاح الشحن
-- انسخ الملف كامل → Supabase Dashboard → SQL Editor → Run
-- Project: ikryeyqrithikabwidov
--
-- القاعدة:
--   • أقل من 3 منتجات → شحن حسب المحافظة (shipping_rates)
--   • 3 منتجات فأكثر → شحن مجاني
--   • عرض روتين كامل (حتى لو منتجين) → شحن مجاني
-- =============================================================================

-- Restore governorate shipping on create_guest_order.
-- Free shipping when: 3+ product units, OR cart is a pure ad-routine
-- (all lines share one known bundle_slug) — matches lib/chatEngine.js.
-- Bundle list-price pricing from 110 unchanged.
...
begin
  ...
  -- Free shipping: 3+ units, or pure routine offer (even 2-SKU ads)
  select coalesce(sum(qty), 0) into v_item_qty from _order_lines;
  select case
    when bool_and(bundle_slug is not null)
     and count(distinct bundle_slug) = 1
     and min(bundle_slug) in ('post-laser-glow', 'brightening-routine', 'face-and-body')
    then min(bundle_slug)
    else null
  end into v_bundle_only
  from _order_lines;

  if v_item_qty >= 3 or v_bundle_only is not null then
    v_computed_shipping := 0;
  else
    v_computed_shipping := null;
    if p_governorate is not null then
      select cost into v_computed_shipping
        from shipping_rates where governorate = p_governorate limit 1;
    end if;
