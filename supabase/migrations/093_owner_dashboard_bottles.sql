-- Owner dashboard: expose bottle counts (Excel Total Bottles) on pharmacy AR.

-- Normalize order channel for owner KPIs (ManyChat + session fallback).
create or replace function public.owner_order_channel(
  p_chat_channel text,
...
as $$
  select case
    when ch in ('messenger', 'facebook') then 'messenger'
    when ch = 'instagram' then 'instagram'
    when ch = 'whatsapp' then 'whatsapp'
    when ch = 'manychat' then coalesce(
      (
        select case
          when cbs.chat_channel in ('messenger', 'instagram', 'whatsapp') then cbs.chat_channel
          else null
        end
        from chat_bot_sessions cbs
        where cbs.session_id = p_chat_session_id
        limit 1
      ),
      case
        when p_chat_session_id like 'messenger:%' then 'messenger'
        when p_chat_session_id like 'instagram:%' then 'instagram'
        when p_chat_session_id like 'whatsapp:%' then 'whatsapp'
        else 'manychat'
      end
    )
    when ch is null or ch = '' or ch = 'web' then
      case
        when p_chat_session_id like 'messenger:%' then 'messenger'
        when p_chat_session_id like 'instagram:%' then 'instagram'
        when p_chat_session_id like 'whatsapp:%' then 'whatsapp'
        when p_chat_session_id like 'manychat:%' then 'manychat'
        else 'web'
      end
    else ch
  end
  from (select lower(nullif(trim(p_chat_channel), '')) as ch) x;
$$;

grant execute on function public.owner_order_channel(text, text) to authenticated;
...
begin
  ...
  select json_build_object(
    ...
    'orders_facebook_all', (
      ...
      where status <> 'cancelled'
        and public.owner_order_channel(o.chat_channel, o.chat_session_id) = 'messenger'
    ),
    ...
    'orders_instagram_all', (
      select count(*)::int from orders o
      where status <> 'cancelled'
        and public.owner_order_channel(o.chat_channel, o.chat_session_id) = 'instagram'
    ),
    ...
    'orders_manychat_all', (
      select count(*)::int from orders o
      where status <> 'cancelled'
        and public.owner_order_channel(o.chat_channel, o.chat_session_id) = 'manychat'
    ),
    'orders_manychat_month', (
      select count(*)::int from orders o
      where status <> 'cancelled'
        and created_at >= v_start and created_at < v_end
        and public.owner_order_channel(o.chat_channel, o.chat_session_id) = 'manychat'
    ...
    'orders_social_month', (
      ...
      where status <> 'cancelled'
        and created_at >= v_start and created_at < v_end
        and public.owner_order_channel(o.chat_channel, o.chat_session_id) in ('messenger', 'instagram', 'manychat')
    ),
    'orders_social_all', (
      select count(*)::int from orders o
      where status <> 'cancelled'
  ...
  select json_build_object(
    ...
    -- Excel Remain sum (signed): Total − Collection for every non-cancelled invoice
    ...
    'bottles_all', coalesce((
      select sum(coalesce(bottles, 0)) from crm_pharmacy_invoices where status <> 'cancelled'
    ), 0),
    'bottles_month', coalesce((
      select sum(coalesce(bottles, 0)) from crm_pharmacy_invoices
      where status <> 'cancelled'
        and invoice_date >= v_start::date and invoice_date < v_end::date
    ), 0)
  ) into v_pharmacy_ar;
  ...
  from (
    select
      ...
      coalesce(bottles, 0) as bottles,
      status,
      coalesce(price_list, 'new') as price_list,
      is_legacy,
      created_at,
      case
        when status = 'paid' or (total - amount_paid) <= 0 then 'paid'
    ...
    select * from (
      ...
      from (
        values
          ...
          ('manychat', 'فيسبوك / إنستجرام (ManyChat)', 'fa-share-nodes', 4),
          ('whatsapp', 'WhatsApp', 'fa-whatsapp', 5)
      ) as ch(key, label_ar, icon, sort)
      left join (
        select public.owner_order_channel(chat_channel, chat_session_id) as channel, count(*)::int as cnt
        from orders
      ...
      left join (
        select
          case
            ...
            when cbs.chat_channel = 'manychat' then 'manychat'
            ...
            when cbs.session_id like 'manychat:%' then 'manychat'
            else 'web'
          end as channel,
  ...
  from (
    select
      ...
      total,
      status,
      public.owner_order_channel(chat_channel, chat_session_id) as chat_channel,
      created_at
    from orders
    where status <> 'cancelled'
