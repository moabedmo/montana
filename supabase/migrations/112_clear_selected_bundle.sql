-- Allow clearing sticky selected_bundle / sku context.
-- Convention for p_selected_bundle:
--   NULL  → leave existing value (field omitted / unchanged)
...
begin
  ...
  on conflict (session_id) do update set
    ...
    selected_bundle = case
      -- NULL arg = leave sticky value alone (caller omitted field)
      when p_selected_bundle is null then chat_bot_sessions.selected_bundle
      -- Empty string = explicit clear
      when length(trim(p_selected_bundle)) = 0 then null
      else trim(p_selected_bundle)
    end,
    updated_at = now();
end $$;
