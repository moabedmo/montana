...
/**
 * Same contract as completeJson, with one image attached.
 * @param {{system: string, user: string, image: {data: string, mediaType: string}, maxTokens?: number}} args
 * @returns {Promise<object|null>}
 */
async function completeVisionJson({ system, user, image, maxTokens = 700 }) {
  ...
  if (anthropic) {
    try {
      ...
      console.warn('[llm] Claude vision failed, trying Gemini:', e.message);
    }
  }
