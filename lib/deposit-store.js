...
async function listPendingProofs(limit) {
  var supabase = trySupabase();
  if (!supabase) {
    return Object.keys(memory.proofs)
      .map(function (id) { return memory.proofs[id]; })
      .filter(function (p) { return p && p.status === 'pending'; })
      .slice(0, limit || 30);
  }
  var res = await supabase
    .from('deposit_proofs')
    ...
    .limit(limit || 30);
  if (res.error) throw res.error;
  return (res.data || []).map(rowToProof);
}
...
module.exports = {
  ...
  listPendingProofs: listPendingProofs,
