...
- عروض الروتين الحقيقية فقط: (1) ما بعد الليزر **618** (2) التفتيح الكامل **777** (3) الوش والجسم **558** — نفس أسعار الموقع + شحن مجاني. **ممنوع** تقولي 549/699/499 أو "بدل 618/777/558" — مفيش خصم تاني على العرض. ممنوع سيرم/واقي شمس لو مش في المنتجات.
...
function enforceBriefReply(text) {
  ...
  if (/(MON-|complete-order\.html|رقم الطلب|تم تسجيل أوردرك|إجمالي المنتجات|تحبي تسجّلي الأوردر معايا|منتجاتنا المتاحة|المكونات:|────|شحن مجاني\) على كل عرض|على أي روتين|montana\.com\.eg|ابعتيلي في \*\*رسالة واحدة\*\*)/i.test(raw)) {
    return raw;
  }
  const maxChars = 420;
...
function isDarkAreaBrighteningAsk(message) {
  ...
  if (!area.test(t)) return false;
  // Not ours: itching, hair, smell, sweat. Anchored to word starts — as bare
  // substrings "الم" sits inside "المناطق" and "شعر" inside plenty else, so
  // this quietly rejected the very messages it was meant to let through.
  // Whole words only. "الم" as a prefix match sits inside "المناطق" and killed
  // the exact messages this is meant to serve; "شعر" does the same elsewhere.
...
async function smartGeminiFallback(message, sid, chatHistory, prefix, selectedBundleKey) {
  ...
  if (isPriceQuestion(message) || isRoutineOrOfferPriceAsk(message) || isVagueOfferQuestion(message)) {
    ...
    // Never dump the full catalog for bare price asks — list the 3 ad offers
    return (
      `${prefix}` + formatAllRoutineOffersList('تحبي سعر أنهي عرض يا فندم؟ (**شحن مجاني** على كل عرض):')
    );
  }
...
function discountHaggleReply(sid, chatHistory, selectedBundleKey) {
  const bundle =
...
/** Customer sent a photo/sticker (Messenger/IG/ManyChat) → pitch the 3 routine offers. */
function imageOffersReply() {
  return (
    'وصلتني الصورة يا فندم 💜\n' +
    formatAllRoutineOffersList('العروض المتاحة (**شحن مجاني** على كل عرض):') +
    '\n\nاختاري بالاسم أو الرقم (١ ليزر · ٢ تفتيح · ٣ وش وجسم) 💜'
  );
}
...
/** First-touch welcome (ad / empty chat) — list routines + free ship, no order-nag. */
function routinesWelcomeReply() {
  return (
    'أهلاً بيكِ في مونتانا 🌿\n' +
    formatAllRoutineOffersList('عندنا **شحن مجاني** على أي روتين من العروض دي:') +
    '\n\nاختاري بالاسم أو الرقم (١ ليزر · ٢ تفتيح · ٣ وش وجسم) وقوليلي أنهي روتين 💜'
  );
}
...
function whichOfferPriceAskReply() {
  const post = getBundleByKey('post-laser');
  const bright = getBundleByKey('brightening');
  const face = getBundleByKey('face-body');
  return (
    'عندنا 3 عروض بروتين كامل وشحن مجاني يا فندم 💜\n\n' +
    `١) **${post.name}** — **${post.bundlePrice}** ج\n` +
    `المنتجات: ${post.productLabels.join(' + ')}\n\n` +
    `٢) **${bright.name}** — **${bright.bundlePrice}** ج\n` +
    `المنتجات: ${bright.productLabels.join(' + ')}\n\n` +
    `٣) **${face.name}** — **${face.bundlePrice}** ج\n` +
    `المنتجات: ${face.productLabels.join(' + ')}\n\n` +
    'تحبي أنهي عرض؟ قولي الاسم أو الرقم'
  );
}
...
async function handleInboundMessage({
  ...
  if (
    ...
    if (isOfferPriceFollowUpAsk(message, lastBotEarly) || wantsAllOffersList(message)) {
  ...
  if (
    ...
    return finish({
      reply: formatAllRoutineOffersList(
  ...
  // Pregnancy / breastfeeding → her doctor decides, never the bot. Left to the
  // model it answered "منتجاتنا كلها آمنة ومناسبة للحوامل" — a safety claim the
  // brand has never made, about products containing salicylic acid. The correct
  // reply existed all along but sat inside the block LLM-first skips.
  if (message && isPregnancyQuestion(message)) {
    return finish(pregnancySuitabilityReply(sid, chatHistory), message);
  }
  ...
  if (isLlmFirstEnabled() && message) {
    ...
    // Customer pasted name+phone+address with items in cart
  ...
  if (!skipScriptedMaze && message) {
    ...
    if (isDiscountHaggleAsk(message)) {
      return finish({
        reply: discountHaggleReply(sid, chatHistory, selectedBundleKey),
        sessionId: sid,
      }, message);
    }
