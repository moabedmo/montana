...
const { findMispricedProducts, truePriceListReply, discountedPriceTable } = require('./priceGuard');
...
function formatSingleProductPriceLine(p, promoPercent = 0) {
  ...
  const after = live ? price - Math.round((price * promoPercent) / 100) : price;
  let offer;
  if (live) {
    offer = `سعره **${after}** جنيه بعد خصم الـ **${promoPercent}%** (بدل ${price} جنيه)`;
  } else if (p.old_price && p.old_price > p.price) {
    offer = `سعره حاليًا ${price} جنيه بدلًا من ${Math.round(p.old_price)} جنيه`;
  } else {
    offer = `سعره ${price} جنيه`;
  }
...
async function handleInboundMessage({
  ...
  if (!chatSessions[sid]) {
    ...
    // The percentage alone left the model doing the subtraction, and asked
    // "هيبقى بكام بعد الخصم؟" it answered with the shelf price. It gets the
    // finished figures instead.
    const promoPricesForPrompt = activePromo.active
      ? await discountedPriceTable(activePromo.percent) : '';
      ...
      systemInstruction: {
        parts: [{
          ...
          text: [
            ...
            stockistPrompt || '',
            promoForPrompt
              ? `## عرض شغال دلوقتي\n${promoForPrompt}${promoPricesForPrompt ? `\n\n${promoPricesForPrompt}` : ''}`
              : '',
          ].filter(Boolean).join('\n\n'),
        }],
      },
