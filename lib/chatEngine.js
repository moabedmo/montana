...
const { getPromo, promoDiscountFor, promoLine, promoPromptBlock } = require('./promo');
const { voiceReplyFor } = require('./voiceReply');
const { sendManyChatAudio } = require('./manychatApi');
const { findMispricedProducts, truePriceListReply } = require('./priceGuard');
...
- **الصوت**: لو طلبت أكلمها بصوتي أو قالت مش بتعرف تقرا — النظام بيبعت الرد صوت تلقائيًا. ردي عادي ومتقوليش إنك بتردي كتابة بس ولا تعتذري. لو حبيتِ قولي «سامعاني كده يا فندم؟ 💜».
 ...
 * several of them run to checkout. Those were real orders: they landed in the
 * admin, they woke the owner's Telegram, and each one took its quantities out
 * of products.stock — which deleting the row afterwards does not give back.
 *
 * The eval names its sessions "messenger:eval-<case>-<timestamp>". Nobody
 * called eval- is a customer.
 */
function isEvalSession(sid) {
  return /(^|:)eval-/.test(String(sid || ''));
}
...
async function create_order(args, sid, channel) {
  ...
  // The eval gets the whole conversation and none of the consequences: no row
  // in the admin, no Telegram alert, no stock taken off. It asserts on what
  // the bot says, and the bot says the same thing either way.
  const evalRun = isEvalSession(sid);
  const { data: order, error: oErr } = evalRun
    ? { data: { order: { order_number: `EVAL-${Date.now().toString().slice(-6)}` } }, error: null }
    : await sb.rpc('create_guest_order', {
  ...
  try {
    ...
    if (tgConfigured() && orderNumber && !evalRun) {
...
// The single-product price answer is templated, not written by the model, so
// the campaign has to be applied here too. Told the discount was automatic and
// then asked "هيبقى بكام؟", the bot was quoting the shelf price — the customer
// heard about a discount and never heard what she pays.
function formatSingleProductPriceLine(p, promoPercent = 0) {
  const price = Math.round(Number(p.price));
  const offer = p.old_price && p.old_price > p.price
    ? `سعره حاليًا ${price} جنيه بدلًا من ${Math.round(p.old_price)} جنيه`
    : `سعره ${price} جنيه`;
  // The shelf price stays the price — the campaign is a discount on the order,
  // not a markdown, and the website still shows 249. Replacing the number here
  // made one reply say 199 while every other template said 249, which is a
  // worse answer than either. The figure she pays is added, not substituted.
  const live = promoPercent > 0 && promoPercent < 100;
  const after = live
    ? `\n• وبعد خصم الـ **${promoPercent}%** بيبقى **${price - Math.round((price * promoPercent) / 100)}** جنيه على الأوردر`
    : '';
...
async function trySingleSkuPriceReply(message, sid, chatHistory, selectedBundleKey) {
  ...
  if (matched.length === 1) {
    const pct = (await getPromo()).percent || 0;
    return {
      reply: formatSingleProductPriceLine(matched[0], pct) + singleSkuShippingTip(1),
      sessionId: sid,
      keepCta: true,
    };
  }
  if (matched.length > 1) {
    const pct = (await getPromo()).percent || 0;
    return {
      reply: matched.map((p) => formatSingleProductPriceLine(p, pct)).join('\n\n')
...
async function tryPriceQuestion(message, sid, chatHistory, selectedBundleKey, selectedProductSlug) {
  ...
  // read once for the whole reply — getPromo() caches, but the price on two
  // lines of the same answer must come from the same read either way
  const promoPct = (await getPromo()).percent || 0;
  const withUnitPricesAndShipTip = (matchedProducts) => {
    const list = (matchedProducts || []).slice(0, 2);
    const lines = list.map((p) => formatSingleProductPriceLine(p, promoPct));
...
function discountHaggleReply(sid, chatHistory, selectedBundleKey, promo) {
  ...
  // She is haggling over a routine, and the routines are the one thing the
  // campaign does not touch. Saying "الخصم بيتحسب لوحده" right under a bundle
  // price promised her a reduction checkout would not give.
  if (promo?.active) {
    return (
      `${promoLine(promo)}\n` +
      `${priceLine}\n` +
      `عروض الروتين سعرها ثابت والخصم مش عليها — بس الشحن مجاني عليها 🎁\n\n` +
      `لو حابة نكمّل قولي «نكمل» 🌿`
    );
  }
  return (
...
/**
 * "كلميني بصوتك عشان ما بعرفش اقرا قوي" — a customer telling us she cannot
 * read the messages we keep sending her.
 *
 * A real one arrived mid-checkout and got the same written address prompt four
 * times, because the details step answers anything it cannot parse with the
 * prompt again. This belongs with a person, not with a fifth copy.
 */
function wantsVoiceOrCall(message) {
  const t = String(message || '').trim();
  if (!t || t.length > 160) return false;
  // "رن" is anchored to a word start: without that it also matches inside
  // ordinary words such as قرنية
  return /(كلمين[يى]|اتصل[وي]?\s*(بي|بيا|عليا)|(?:^|\s)رن[ّيى]*ل?[يى]|بصوت|صوت[يىك]|فويس|voice|call\s*me)/i.test(t)
    || /(م(ا)?\s*بعرفش?\s*اقرا|مش\s*(بعرف|عارفة)\s*اقرا|نظر[يى]\s*ضعيف|مش\s*شايفة)/i.test(t);
}
...
async function advanceCheckoutWizard(sid, message, channel, classified = null, chatHistory = []) {
  ...
  if (wiz.step === 'details') {
    ...
    // A turn that captured nothing means the same prompt is about to go out
    // again. Unbounded, that is what produced four identical messages to one
    // customer — including to the two where she was asking for help, not
    // withholding an address.
    const asks = gotBits.length ? 0 : (wiz.asks || 0) + 1;
    checkoutWizards[sid] = { step: 'details', data, asks };

    if (!gotBits.length && (asks >= 3 || wantsVoiceOrCall(message))) {
      checkoutWizards[sid] = { step: 'details', data, asks: 0 };
      return {
        reply:
          `تحت أمرك يا فندم 💜 هيتواصل معاكِ حد من الفريق يكمّل الطلب معاكِ.\n` +
          `أو كلّمينا على **${CS_SUPPORT_PHONE}** في أي وقت.`,
        sessionId: sid,
        needsHuman: true,
      };
    }
    ...
    const prefix = gotBits.length
      ...
      : '';
    // Second time of asking, say it differently and show what one looks like.
    // Repeating a sentence verbatim reads as a broken bot, not as patience.
    if (!gotBits.length && asks === 2) {
      const hasAddr2 = !!(data.address && isUsableDeliveryAddress(data.address));
      return {
        reply: hasAddr2
          ? checkoutDetailsAskReply(data)
          : 'معلش يا فندم 💜 محتاجة العنوان بشكل أوضح شوية — مثال:\n'
...
async function handleInboundMessage({
  ...
  const finish = async (payload, userMsg = message) => {
    ...
    // A price attached to the wrong product is the one mistake a shop cannot
    // make. The rule lives in the prompt and the model has broken it: it once
    // quoted the whitening cream at 369, which is the post-laser cream's price.
    // Checked against the rows themselves, not asked of the model again.
    if (payload?.reply && /\d/.test(payload.reply)) {
      try {
        const promo = await getPromo();
        const wrong = await findMispricedProducts(payload.reply, promo.active ? promo.percent : 0);
        if (wrong.length) {
          const corrected = await truePriceListReply(promo.active ? promo.percent : 0);
          console.error('[price-guard] blocked:',
            wrong.map((w) => `${w.slug} said ${w.said} not ${w.actual}`).join(', '));
          if (corrected) payload.reply = corrected;
        }
      } catch (err) {
        // never let the guard itself cost a customer her answer
        console.warn('[price-guard] check failed:', err.message);
      }
    }
    ...
    // Someone who asked to be spoken to gets the same reply as a voice note.
    // It is attached to the written reply, never instead of it, and a failure
    // to synthesise leaves the conversation exactly as it was.
    if (payload?.reply && wantsVoiceOrCall(userMsg)) {
      const audioUrl = await voiceReplyFor(payload.reply);
      if (audioUrl) {
        payload.audioUrl = audioUrl;
        // The ManyChat flow has no audio block bound to audioUrl, so on those
        // channels the file would be generated and never heard. Push it through
        // the Send API the order confirmation already uses — no tag, because
        // this only ever follows a message she just sent.
        if (sid.startsWith('manychat:')) {
          const sent = await sendManyChatAudio(
            sid.slice('manychat:'.length),
            audioUrl,
            ['messenger', 'instagram'].includes(channel) ? channel : undefined,
          );
          if (!sent.ok) console.warn('[voice] manychat send failed:', sent.error);
        }
      }
    }
    return payload;
  ...
  // Haggling over a routine. The campaign does not touch the offers, so left
  // to the model this became "777 عليه 20% يبقى 621.6" — a discount checkout
  // refuses, in piastres this shop does not use.
  //
  // There is a copy of this inside the `!skipScriptedMaze` block below. That
  // block does not run in production and the copy has never answered anybody;
  // this is the one that does.
  // isPriceHaggleAsk matches on "غالي" alone, so "غالي جدا متشكره خلاص مش
  // هينفع اطلبه" reads as haggling. She is leaving, and the soft close below
  // owns that — pitching her the fixed price instead is the wrong answer.
  if (message && isPriceHaggleAsk(message) && !isWalkAwayDecline(message)) {
    const promoNow = await getPromo();
    const bundleNow = getBundleByKey(selectedBundleKey)
      || resolveSinglePitchedBundle(lastModelMessage(chatHistory))
      || (hasRecentBrighteningContext(chatHistory) ? getBundleByKey('brightening') : null);
    if (bundleNow) {
      return finish({
        reply: discountHaggleReply(sid, chatHistory, selectedBundleKey, promoNow),
        sessionId: sid,
        keepCta: true,
      }, message);
    }
  ...
  if (!chatSessions[sid]) {
    ...
    const promoForPrompt = activePromo.active ? promoPromptBlock(activePromo) : '';
...
module.exports = {
  // Exported for scripts/test-checkout-repeat-loop.js — a customer who cannot
  // read the prompt has to be routed to a person, and the test needs to check
  // that detector against the phrasings it actually arrives in.
  wantsVoiceOrCall,
