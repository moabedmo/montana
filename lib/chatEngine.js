...
function isPregnancyQuestion(message) {
  const t = prepareCustomerText(expandFrancoAndTypos(String(message || '')));
  if (!t || t.length > 140) return false;
  const hasPreg = /(حوامل|حامل|الحمل|الحامل)/i.test(t);
  if (!hasPreg) return false;
  return /(نفع|ينفع|مناسب|يصلح|تنفع|آمن|ا?مان|السلامه|ا?لسلامه|استخدام)/i.test(t);
}
...
function pregnancySuitabilityReply(sid, chatHistory) {
  ...
  let reply =
    'بالنسبة للحوامل يا فندم: الروتين ده للاستخدام الموضعي فقط.\n' +
    'عشان الأمان، الأفضل تستشيري طبيبك/صيدلي قبل الاستخدام، وابدئي بجزء صغير ووقفي لو حصل أي تهيّج.';

  if (inCart) {
    reply += '\n\nالروتين موجود في أوردرك — تحبي نكمل تسجيل الأوردر؟';
  } else if (pitched && bundle) {
    reply += '\n\nتحبي نكمل تسجيل الروتين؟';
  }
