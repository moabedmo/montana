...
function isDarkAreaBrighteningAsk(message) {
  ...
  const area = /(تحت\s*ال[اإ]بط|اندر\s*ارم|الاكواع|الاكو[اع]|الركب|الرقبه|بيكين[يى]|bikini|المناطق\s*الحساس|اماكن\s*حساس)/i;
  if (!area.test(t)) return false;
  // Only when it's about colour, not irritation/hair/smell — those aren't ours
...
async function handleInboundMessage({
  ...
  if (message && !checkoutWizards[sid] && isDarkAreaBrighteningAsk(message)) {
