...
- اللهجة المصرية العامية بس — ممنوع أي كلمة فصحى. مثال: قولي "ممكن أساعدك إزاي يا فندم؟" مش "كيف أقدر أساعدك؟". ممنوع "كيف"، "ماذا"، "هل"، "لماذا" — استخدمي "إزاي"، "إيه"، "ولا"، "ليه".
...
3. عايز يكمل شراء صراحة ("نكمل الطلب"، "جاهز") = النظام بيخيّره بين بيانات في الشات أو لينك الموقع — متقوليش إنك هتسجلي لوحدك.
4. "اوك"/"تمام" بعد "تحبي أسجلك أوردر يا فندم؟" = موافقة على إضافة بس، مش شراء نهائي — add_to_cart، ممنوع تفتحي checkout لوحدك.
...
6. اتعمل أوردر قبل كده في نفس المحادثة وعايز يطلب تاني = لازم تأكيد صريح قبل ما النظام يفتح اختيار الإتمام تاني.
...
10. سؤال عن حالة الطلب ("اتأكد؟"، "وصل فين؟") = استخدمي get_order_status فورًا. ممنوع تمامًا تقولي إن الطلب اتأكد أو اتسجل من دماغك — لازم من الأداة دي بس.
...
لما العميل يبقى جاهز يكمل الأوردر، النظام بيخيّره تلقائيًا بين: (1) يبعت بياناته هنا في الشات، أو (2) يكمّل على الموقع بلينك فيه المنتجات اللي اختارها + برنامج النقاط. ممنوع تقولي "هفتح فورم" أو "هبعت لينك" من غير ما النظام يعملها. وممنوع تختلقي رقم أوردر — التسجيل الحقيقي بيحصل بس بعد البيانات (في الشات أو على الموقع).

## سياسات المتجر (لو سأل تحديدًا):
توصيل 2-3 أيام (شحن يختلف حسب المحافظة)، إرجاع خلال 14 يوم، دفع: كاش عند الاستلام (بدون مقدّم) أو تحويل كامل بالمحفظة/إنستاباي مع إيصال.
...
const tools = [{
  functionDeclarations: [
    ...
    },
    {
      ...
      description: 'يرجع حالة آخر أوردر حقيقي للعميل من قاعدة البيانات. استخدميها لو العميل سأل عن حالة طلبه أو "هل تم التأكيد".',
      parameters: { type: SchemaType.OBJECT, properties: {} }
    }
  ]
}];
...
function maybeNudgeCheckoutOnThanks(message, sid) {
  ...
  if (isExplicitCheckoutIntent(message)) {
    return offerCheckoutChoice(sid);
  }
...
function isCheckoutMethodChat(message) {
  ...
  return /(في\s*الشات|هنا\s*في\s*الشات|ابعت.*(بيانات|هنا)|بياناتي\s*هنا|اكتب.*(هنا|الشات)|بالشات|جوه\s*الشات)/i.test(t);
}

function isCheckoutMethodSite(message) {
  const t = String(message || '').trim();
  if (/^(2|٢|اتنين|ثنين)([\s.،!؟?]|$)/.test(t)) return true;
  return /(الموقع|اللينك|الرابط|لينك|رابط|سجّ?ل|تسجيل|النقاط|برنامج\s*النقاط|على\s*الويب|checkout)/i.test(t);
}

const SITE_URL = 'https://www.montana.com.eg';

function offerCheckoutChoice(sid) {
  const summary = formatCartSummary(sid);
  checkoutWizards[sid] = { step: 'await_method' };
  return {
    reply:
      `تمام يا فندم 💜\n\n${summary}\n\n` +
      `تقدري تكمّلي الطلب بطريقتين:\n\n` +
      `1️⃣ **ابعتي بياناتك هنا في الشات** (الاسم · الموبايل · العنوان · المحافظة)\n\n` +
      `2️⃣ **كمّلي على الموقع** — هبعتلك لينك فيه المنتجات اللي اخترتيها، وتقدري تستفيدي من **برنامج نقاط مونتانيا** (نقطة لكل 10 ج.م + خصومات) 🎁\n\n` +
      `ردي بـ **1** أو **2**`,
    sessionId: sid,
  };
}
...
function sendCheckoutLink(sid) {
  ...
  return {
    reply:
      `تمام يا فندم 💜 ده لينك إتمام الطلب على موقع مونتانيا:\n` +
      `هتلاقي فيه **المنتجات اللي اخترتيها جاهزة**، وتكمّلي بياناتك بسهولة، وكمان تستفيدي من **برنامج نقاط مونتانيا** 🎁 (نقطة لكل 10 ج.م — مربوط برقم الموبايل):\n\n` +
      `${url}`,
    sessionId: sid,
  };
}
...
function isValidEgPhone(raw) {
  return /^01\d{9}$/.test(normalizePhoneDigits(raw));
}
...
async function matchGovernorateName(input) {
  ...
  if (!t) return null;
  const exact = governorates.find((g) => g.governorate === t);
  if (exact) return exact;
  const lower = t.toLowerCase();
  const fuzzy = governorates.find((g) => {
    const name = String(g.governorate || '');
    return name.includes(t) || t.includes(name) || name.toLowerCase().includes(lower);
  });
  return fuzzy || null;
}
...
async function tryAdvanceCheckoutConfirm(sid, message) {
  ...
  if (isCheckoutConfirmYes(message)) {
    delete checkoutWizards[sid];
    return { ...offerCheckoutChoice(sid), clearConfirm: true };
  }
...
async function advanceCheckoutWizard(sid, message, channel) {
  const wiz = checkoutWizards[sid];
  if (!wiz) return null;
  ...
  if (!cart.length && wiz.step !== 'await_confirm') {
    delete checkoutWizards[sid];
    return { reply: 'الأوردر فاضي دلوقتي — قوليلي تحبي تطلبي إيه 💜', sessionId: sid };
  }

  // Step: choose chat vs website
  if (wiz.step === 'await_method') {
    if (isCheckoutMethodSite(message)) {
      delete checkoutWizards[sid];
      return sendCheckoutLink(sid);
    }
    if (isCheckoutMethodChat(message)) {
      if (channelOf(sid) === 'web') {
        delete checkoutWizards[sid];
        return {
          reply: `تمام يا فندم 💜 املي بياناتك تحت — أو لو حابة الموقع قوليلي **2**\n\n${formatCartSummary(sid)}`,
          sessionId: sid,
          showOrderForm: show_checkout_form(sid),
        };
      }
      checkoutWizards[sid] = { step: 'name', data: {} };
      return {
        reply: 'تمام ✍️ ابعتي **اسمك بالكامل** عشان نسجّل الأوردر',
        sessionId: sid,
      };
    }
    // Not a clear 1/2 — keep waiting
    return {
      reply: 'اختاري من فضلك:\n1️⃣ بيانات في الشات\n2️⃣ لينك الموقع + برنامج النقاط\n\nردي بـ **1** أو **2**',
      sessionId: sid,
    };
  }
  ...
  if (wiz.step === 'name') {
    ...
    if (name.length < 2 || /^(1|2|١|٢)$/.test(name)) {
      return { reply: 'اكتبي اسمك بالكامل لو سمحتي 😊', sessionId: sid };
    }
    checkoutWizards[sid] = { step: 'phone', data: { ...wiz.data, name } };
    return { reply: `تشرفنا يا ${name.split(' ')[0]} 💜\nابعتي **رقم الموبايل** (01xxxxxxxxx)`, sessionId: sid };
  }

  if (wiz.step === 'phone') {
    if (!isValidEgPhone(message)) {
      return { reply: 'الرقم مش مضبوط — ابعتي موبايل مصري 11 رقم يبدأ بـ 01', sessionId: sid };
    }
    const phone = normalizePhoneDigits(message);
    checkoutWizards[sid] = { step: 'address', data: { ...wiz.data, phone } };
    return { reply: 'تمام ✅ ابعتي **العنوان بالتفصيل** (المنطقة + الشارع + علامة مميزة)', sessionId: sid };
  }

  if (wiz.step === 'address') {
    const address = String(message || '').trim();
    if (address.length < 6) {
      return { reply: 'العنوان قصير شوية — اكتبي المنطقة والشارع بالتفصيل 📍', sessionId: sid };
    }
    checkoutWizards[sid] = { step: 'governorate', data: { ...wiz.data, address } };
  ...
  if (wiz.step === 'governorate') {
    const gov = await matchGovernorateName(message);
    if (!gov) {
      const hint = await governorateHintList();
      return {
        reply: `المحافظة دي مش ظاهرة عندي بنفس الاسم — جرّبي اسم المحافظة زي ما في القائمة:\n${hint}`,
        sessionId: sid,
      };
    }
    ...
    if (order.error) {
      return { reply: `في مشكلة: ${order.error}\nتحبي تحاولي تاني؟ قولي **نكمل**`, sessionId: sid };
    }
    return {
      reply:
        `تمام يا فندم! ✅ اتسجل طلبك برقم **${order.order_number}**\n` +
        `الإجمالي ${Math.round(order.total)} جنيه (شامل الشحن لمحافظة ${gov.governorate}) — الدفع كاش عند الاستلام.\n` +
        `هنراجع الطلب ونكلمك قريب 💜\n\n` +
        `💡 لو حابة تستفيدي من **نقاط مونتانيا** في الطلبات الجاية، استخدمى نفس رقم الموبايل على الموقع.`,
      sessionId: sid,
      order: {
...
async function runFunctionCall(name, args, sid) {
  ...
  if (name === 'get_order_status') {
    ...
    if (!order) return { found: false, message: 'مفيش أوردر مسجل على الجلسة دي' };
    return { found: true, order_number: order.order_number, status: order.status, total: order.total };
  }
...
async function handleInboundMessage({ sid, message, formSubmit, channel }) {
  ...
  if (formSubmit) {
    ...
    return finish({
      reply: `تمام يا فندم! ✅ اتسجل طلبك برقم **${order.order_number}**، والإجمالي ${Math.round(order.total)} جنيه — الدفع كاش عند الاستلام. هنراجع الطلب ونكلمك قريب لتأكيد التوصيل 💜`,
      sessionId: sid,
      order: {
        order_number: order.order_number,
        total: order.total,
  ...
  // Checkout choice / in-chat details wizard — handle BEFORE intent/Gemini
  // so "1" / "2" / name / phone aren't misclassified as product talk.
  if (checkoutWizards[sid] && checkoutWizards[sid].step !== 'await_confirm') {
    const wizResult = await advanceCheckoutWizard(sid, message, channel);
    if (wizResult) {
  ...
  const { intent, products: wantedNames } = await classifyIntent(message, lastBot, productNames);
  console.log('[DIAG intent]', JSON.stringify({ sid, message, intent, wantedNames }));
  ...
  if (intent === 'ADD_CONFIRM' && wantedNames.length) {
    ...
    const lines = [];
    if (added.length) lines.push(`تمام يا فندم 💜 أضفت ${added.join(' و ')} لأوردرك ✅`);
    if (unavailable.length) lines.push(`${unavailable.join(' و ')} غير متوفر حالياً للأسف`);
    if (lines.length) return finish({ reply: lines.join('\n'), sessionId: sid });
  }

  if (intent === 'CHECKOUT_READY' || intent === 'SEND_CHECKOUT_LINK') {
    const cart = sessionCarts[sid] || [];
    if (!cart.length) {
      return finish({ reply: 'أوردرك فاضي يا فندم 💜 قوليلي تحبي تطلبي إيه', sessionId: sid });
    }
    return finish(offerCheckoutChoice(sid));
  }
  ...
  if (/(هفتحلك|هبعتلك|بعتلك|فتحتلك|هارسلك|هرسلك)\s*(ال)?(فورم|لينك)/i.test(result.response.text() || '')) {
    const cart = sessionCarts[sid] || [];
    if (cart.length) return finish(offerCheckoutChoice(sid));
    return finish({ reply: 'أوردرك فاضي يا فندم 💜 قوليلي تحبي تطلبي إيه', sessionId: sid });
  }
...
module.exports = { handleInboundMessage, channelOf, chatSessions, sessionCarts, getCartForSid, getOrderForSid, getPointsBalance };
