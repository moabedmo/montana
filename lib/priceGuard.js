...
/**
 * What each product actually costs once the campaign is taken off, for the
 * prompt.
 *
 * Asked "هيبقى بكام بعد الخصم؟" the bot answered with the shelf price, because
 * it was told a discount exists and left to work out the rest. It is given the
 * arithmetic done instead — the same rule as every other price here: read it,
 * do not derive it.
 */
async function discountedPriceTable(promoPercent = 0) {
  if (!(promoPercent > 0 && promoPercent < 100)) return '';
  const prices = await productPrices();
  if (!prices) return '';
  const lines = Object.entries(LABEL)
    .filter(([slug]) => prices[slug])
    .map(([slug, label]) => {
      const full = Math.round(Number(prices[slug].price));
      const after = full - Math.round((full * promoPercent) / 100);
      return `- ${label}: ${full} ج، وبعد الخصم ${after} ج`;
    });
  if (!lines.length) return '';
  return `لو سألت عن السعر بعد الخصم قوليلها الرقم ده بالظبط، متحسبيهوش بنفسك:\n${lines.join('\n')}`;
}
...
module.exports = {
  ...
  discountedPriceTable, toWestern, BUNDLE_TOTALS, NAME_VARIANTS,
};
