...
function extractInboundMessages(body) {
  ...
  body.entry.forEach(function (entry) {
    (entry.messaging || []).forEach(function (event) {
      ...
      var platform = body.object === 'instagram' ? 'instagram' : 'facebook';
      var text = event.message.text || '';
      var imageUrl = null;

      if (event.message.attachments && event.message.attachments.length) {
        for (var a = 0; a < event.message.attachments.length; a++) {
          var att = event.message.attachments[a];
    ...
    (entry.changes || []).forEach(function (change) {
      ...
      if (val.item === 'comment' || val.item === 'post') {
        var platform = body.object === 'instagram' ? 'instagram' : 'facebook';
        messages.push({
          platform: platform,
          type: 'comment',
          sender_id: val.from && val.from.id,
          sender_name: val.from && val.from.name,
...
async function replyToComment(commentId, token, text) {
