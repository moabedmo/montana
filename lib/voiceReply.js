...
// Spoken in Egyptian, warmly and unhurried — the shop's own register,
// not a newsreader's. This is a direction to the model, never spoken aloud.
const STYLE = 'اقرأي النص ده بلهجة مصرية عامية، بصوت بنت ودودة وهادية، بنبرة دافية وغير مستعجلة، زي واحدة بتكلم صاحبتها:';
...
function speakable(text) {
  return String(text || '')
    ...
    .replace(/\s*\n+\s*/g, '. ')
    .replace(/[_`#>]/g, '')
    // an em dash between a product and its price is punctuation for the eye
    .replace(/\s*[—–]\s*/g, ' ')
    // "249 ج" is a written abbreviation; said aloud it is the letter gim
    .replace(/(\d)\s*ج(?:\.?م)?(?![\p{L}\d])/gu, '$1 جنيه')
    // "20%" read as a symbol comes out English or not at all
    .replace(/(\d)\s*%/g, '$1 في الميه')
    // emoji and the decorative hairlines are written furniture; a synthesiser
    // either names them or stumbles over them
    .replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}\u{FE0F}\u{2190}-\u{21FF}]/gu, ' ')
    .replace(/─{2,}/g, ' ')
    // stripping furniture leaves punctuation stranded after a space
    .replace(/\s+([،.])/g, '$1')
    .replace(/،\s*(?=[،.])/g, '')
    .replace(/\s{2,}/g, ' ')
    .trim()
...
async function synthesize(text) {
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        // Gemini's TTS takes a spoken direction ahead of the line, the way a
        // voice actor is briefed. Without one the Arabic comes out neutral and
        // formal, which is not how this shop talks to anybody.
